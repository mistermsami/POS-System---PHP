...
<body onload="window.print()">
            ...
            <div class="card printthis" id="printthis">
              ...
              <?php
                ...
                $que = "select count(or_id) as invoiceno from neworder where or_date like'". date("Y-m-d") ."'";
                  ...
                  $query = "select neworder.or_id, employees.e_name, neworder.or_date, neworder.or_time FROM neworder INNER JOIN employees ON employees.e_id = neworder.e_id WHERE neworder.or_id=".$c_id."";

                  $result = $conn->query($query);
                  foreach ($result as $row) {
                    $inodid=$row['or_id'];
                 ?>
              <p>INVOICE NO: <b><?php echo $invoiceno;  ?></b><br>SERIAL NO: <b><?php echo $inodid;  ?></b><br>ORDER TAKEN: <b><?php echo $row['e_name']; ?></b><br><span style="float:left;">DATE: <b><?php echo $row['or_date']; ?>&nbsp;<?php echo $row['or_time']; ?></b></span></p>
  ...
  border-bottom: 1px solid black;
              ...
              <center><table style="">
                ...
                <br>
                <p style="float:left;">_</p>
                </div>
                
              
              </center>
            </div>
            <center><button class="btn btn-primary" onclick="window.location.href='createinvoice1.php'">Back</button>
              <?php 
                    //$query = "select * from neworder where or_id=".$c_id."";
                    //$result = $conn->query($query);
                    //foreach ($result as $row) {
                      ?>
            <!-- <a href="editinvoice.php?id=<?php //echo $row['or_id'] ?>" style="text-decoration: none;"><button class="btn btn-warning">Edit</button></a> -->
          <?php //} ?>

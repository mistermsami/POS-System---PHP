				  ...
				  <div class="card">
				    ...
				    <div id="<?php echo $row['ca_name'] ?>12" class="collapse" aria-labelledby="<?php echo $row['ca_id']; ?>" data-parent="#accordion">
				      <div class="card-body">
				        ...
				        // $stockmid=0;
				        // $stockleft= 0;
				        // $que="select (menue.m_id) as stock_mid, menue.ca_id, stock.s_quantity, stock.sold_quantity from stock inner join menue on stock.m_id= menue.m_id WHERE s_date= '". date("Y-m-d") ."' and menue.ca_id= '".$row['ca_id']."'";
            //     $result= $conn->query($que) or die("Query Error");
            //     foreach ($result as $row) {
            //       $stockmid =$row['stock_mid'];
            //       $stqty=$row['s_quantity'];
            //       $reqty=$row['sold_quantity'];
            //       $stockleft = $stqty - $reqty;

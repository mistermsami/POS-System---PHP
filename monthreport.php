            ...
            <div class="card">
              ...
              <div class="card-body">
                      ...
                      <br>
                        <?php
                        $totalamount=0;
                      $query="SELECT menue.m_itemprice, stock.s_quantity, stock.s_date, stock.s_id FROM stock LEFT JOIN menue ON stock.m_id = menue.m_id where stock.s_date BETWEEN (NOW() - INTERVAL 1 MONTH) AND NOW()";
                    $result= $conn->query($query) or die("Query Error");
                    foreach ($result as $row) {
                 ...
                 <br><br>
                  ...
                  <br>
                <?php
                $query="select COUNT(or_customer) AS oders, or_date FROM neworder WHERE or_date BETWEEN (NOW() - INTERVAL 1 MONTH) AND NOW()";
                $result= $conn->query($query) or die("Query Error");
                foreach ($result as $row) {
                 ?>
                 <div class="row">
              ...
              <br><br>
                  ...
                  <br>
                <?php
                $query="select COUNT(neworder.e_id) AS odertaken,employees.e_name, neworder.or_date FROM neworder INNER JOIN employees ON neworder.e_id=employees.e_id WHERE or_date BETWEEN (NOW() - INTERVAL 1 MONTH) AND NOW() GROUP BY neworder.e_id HAVING COUNT(neworder.e_id)";
                $result= $conn->query($query) or die("Query Error");
                foreach ($result as $row) {
                 ?>
                 <div class="row">
              ...
              <br><br>
                  ...
                  <br>
                <?php
                $query="select sum(ut_price) AS totautility FROM utility WHERE ut_date BETWEEN (NOW() - INTERVAL 1 MONTH) AND NOW()";
                $result= $conn->query($query) or die("Query Error");
                foreach ($result as $row) {

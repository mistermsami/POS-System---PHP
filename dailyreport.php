          ...
          <div class="col-12">
            <div class="card">
              ...
              <div class="card-body">
                ...
                // $query="select menue.m_item, sum(stock.sold_quantity) as soldqty, sum(stock.s_quantity) as inqty FROM stock INNER JOIN menue ON menue.m_id= stock.m_id where stock.s_date = '". date("Y-m-d") ."' group by menue.m_id having sum(stock.s_quantity)";
                $query="select menue.m_item, sum(stock.sold_quantity) as soldqty, sum(stock.s_quantity) as inqty FROM stock INNER JOIN menue ON menue.m_id= stock.m_id where stock.s_date = '". date("Y-m-d") ."' group by menue.m_id";
                $result= $conn->query($query) or die("Query Error");
                foreach ($result as $row) {
                $remainingstock= $row['inqty'] - $row['soldqty'];
            ...
            <div class="card">
              ...
              <div class="card-body">
                      ...
                      <br>
                        <?php
                        $totalamount=0;
                      $query="SELECT menue.m_itemprice, stock.s_quantity, stock.s_date, stock.s_id FROM stock LEFT JOIN menue ON stock.m_id = menue.m_id where stock.s_date= '". date("Y-m-d") ."'";
		                $result= $conn->query($query) or die("Query Error");
		                foreach ($result as $row) {
              ...
              <br><br>
                  ...
                  <br>
                <?php
                $query="select COUNT(neworder.e_id) AS odertaken,employees.e_name, neworder.or_date FROM neworder INNER JOIN employees ON neworder.e_id=employees.e_id WHERE or_date LIKE '". date("Y-m-d") ."%' GROUP BY neworder.e_id HAVING COUNT(neworder.e_id)";
                $result= $conn->query($query) or die("Query Error");
                foreach ($result as $row) {
                 ?>
                 <div class="row">
              ...
              <?php } ?>
                ...
                <?php
                $profit=0;
                $query="select sum(neworder.or_total) as total, sum(promotions.pr_rupees) as pro, sum(neworder.or_disamt) as dis, sum(neworder.or_grandtotal) as gtotal, sum(neworder.or_profit) as profit from neworder INNER JOIN promotions ON promotions.pr_id = neworder.pr_id where neworder.or_date like '". date("Y-m-d") ."%'";
                $result= $conn->query($query) or die("Query Error");
                foreach ($result as $row) {
                  $profit=$row['profit'];
                 ?>
              ...
              <br><br>
                  ...
                  <br>
                <?php
                $query="select sum(ut_price) AS totautility FROM utility WHERE ut_date ='". date("Y-m-d") ."'";
                $result= $conn->query($query) or die("Query Error");
                foreach ($result as $row) {
